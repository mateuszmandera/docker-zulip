...
- `SETTING_EXTERNAL_HOST`: The hostname your users will use to connect to your
  Zulip server. This should be a domain name, not an IP address; if the
  server has no name in DNS, invent one, and configure it in `/etc/hosts`
  on every machine that will access Zulip. If you're testing on your
  laptop, the default of `localhost.localdomain` is great.
